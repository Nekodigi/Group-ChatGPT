...
export default function Page() {
  ...
  const handleSendMessage = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let aiRes = await handleAIResponse(input);
    for (let i = 0; i < 3; i++) {
      if (!aiRes) return;
      await sleep(3000);
      aiRes = await handleAIResponse(aiRes);
    }
  };
  ...
  const handleAIResponse = async (
    ...
    let query = "This is short conversation like messenger or discord.";
    setMessages((messages) => [
      ...messages,
      {
        id: messages.length + 1,
    ...
    if (!input) return;
    try {
      const res = await fetch(`/api/chatgpt?input=${input}&query=${query}`);
